...
!function($){
	...
	if (!ver || ver >= 9 || ver < 0) {
		...
		$(document).ready(function(){
			...
			$("#off-canvas-nav ul.megamenu").children("li.haschild").each(function(i,li){
               $('<div class="aricon" />').appendTo($(li));
               $(li).children(".aricon").bind("click",function(e){
              
               	 cheight =  $(this).prev(".childcontent").children(".childcontent-inner-wrap").height();
               	 e.stopPropagation();
                 e.preventDefault();
			...
			showNav = function (side) {
                ...
                $("#off-canvas-nav ul.megamenu").children("li.haschild.active").removeClass("active").children(".aricon").trigger("click");
